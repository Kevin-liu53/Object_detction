...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        :param imgs:  (batch,img)
        :param bboxes: ground truth的坐标
        :param labels: ground truth的label
        :param scale:
        :return:
        """
        ...
        #预测的位置偏移量(相对于anchor的),每个预测框正负样本的概率,proposal坐标(x1,y1,x2,y2),anchor(x1,y1,x2,y2)to(xi,yi)
        rpn_loc_loss_all, rpn_cls_loss_all, roi_loc_loss_all, roi_cls_loss_all = 0, 0, 0, 0
        for i in range(n):
            bbox = bboxes[i]  # ground truth
            label = labels[i] # ground truth
            rpn_loc = rpn_locs[i] # predict 偏移
            rpn_score = rpn_scores[i] # 预测的得分
            roi = rois[roi_indices==i] # proposal位置
            feature = base_feature[i]#特征图

            # -------------------------------------------------- #
            #   利用真实框和先验框获得建议框网络应该有的预测结果
            #   给每个先验框都打上标签
            #   gt_rpn_loc : anchor 关于 gt box 的缩放系数(dx,dy,dw,dh)     [num_anchors, 4]
            #   gt_rpn_label    [num_anchors, ]
            # -------------------------------------------------- #
            gt_rpn_loc, gt_rpn_label = self.anchor_target_creator(bbox, anchor, img_size)   # anchor 与gt框的偏移,和label
            ...
            rpn_cls_loss = F.cross_entropy(rpn_score, gt_rpn_label, ignore_index=-1)  #ignore_index = -1 忽略label == -1的分数
    ...
    def train_step(self, imgs, bboxes, labels, scale):
        self.optimizer.zero_grad()  #由于pytorch每次结束都不清空梯度,因此需要手动清零,否则会造成梯度累计
...
def _fast_rcnn_loc_loss(pred_loc, gt_loc, gt_label, sigma):
    pred_loc = pred_loc[gt_label>0]  # proposal关于anchor的缩放系数
    gt_loc = gt_loc[gt_label>0]     # gt_box关于chanor的缩放系数
