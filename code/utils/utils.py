...
def bbox2loc(src_bbox, dst_bbox):
    ...
    :param src_bbox:
    :param dst_bbox:
    :return:    从anchor与gt框,求出dx,dy,dw,dh
    """
...
def loc2bbox(src_bbox, loc):
    ...
    :param src_bbox:    anchor坐标（9*h*w,4)
    :param loc:         预测的偏移量dx,dy,dw,dh  (9*h*w,4)  在特征图上
    :return:            （9*h*w,4) proposal的（x1,y1,x2,y2)坐标
    """
    if src_bbox.size()[0] == 0:
        return torch.zeros((0, 4), dtype=loc.dtype)

    #anchor的宽,高,x,y
    src_width = torch.unsqueeze(src_bbox[:, 2] - src_bbox[:, 0], -1)   #(9*h*w,1)
    src_height = torch.unsqueeze(src_bbox[:, 3] - src_bbox[:, 1], -1)  #(9*h*w,1)
    src_ctr_x = torch.unsqueeze(src_bbox[:, 0], -1) + 0.5 * src_width  #（9*h*w,1)   中心坐标x
    src_ctr_y = torch.unsqueeze(src_bbox[:, 1], -1) + 0.5 * src_height #（9*h*w,1)   中心坐标y

    dx = loc[:, 0::4] #取预测坐标的第一列，由于索引会将其展平，而该操作可以将数据以列排  等效于dx.unsqueeze(-1)（9*h*w,1)
    dy = loc[:, 1::4] #（9*h*w,1)
    dw = loc[:, 2::4] #（9*h*w,1)
    dh = loc[:, 3::4] #（9*h*w,1)

    # 求得proposal框的（x_c,y_c,w,h)   x_c,y_c为中心坐标
    ctr_x = dx * src_width + src_ctr_x
    ctr_y = dy * src_height + src_ctr_y
    w = torch.exp(dw) * src_width
    h = torch.exp(dh) * src_height
    dst_bbox = torch.zeros_like(loc)
    # 将propoasal框进行坐标转换 (x_c,y_c,w,h) -> (x1,y1,x2,y2)
...
class AnchorTargetCreator(object):
    """
        :return loc:(n_anchors,4) 每一个anchor 与其对应gt box的缩放系数(da,dy,dw,dh)
                label:(n_anchors,) 每一个anchor 与其对应的gt box序号
    """
    def __init__(self, n_sample=256, pos_iou_thresh=0.7, neg_iou_thresh=0.3, pos_ratio=0.5):
        self.n_sample = n_sample
        self.pos_iou_thresh = pos_iou_thresh
        self.neg_iou_thresh = neg_iou_thresh
        self.pos_ratio = pos_ratio              #正样本的占比

    def __call__(self, bbox, anchor, img_size):
        argmax_ious, label = self._create_label(anchor, bbox)
        if (label>0).any(): #anchor中如果存在一个正样本则将anchor与其对应的gt box求缩放系数(dx,dy,dw,dh)
            loc = bbox2loc(anchor, bbox[argmax_ious])  #这里计算的时候,将那些被舍弃的框,与最后一个gt box求了缩放系数,是不是有点问题
            return loc, label
        else:               # 若所有的anchor中没有一个正样本,则偏移量均设置为0
            return np.zeros_like(anchor), label

    def _calc_ious(self, anchor, bbox):
        """
        :param anchor:   anchor坐标
        :param bbox:     bbox坐标
        :return:        argmax_ious:与每个anchor的iou最大gt 框的序号 (n_anchors,)
                        max_ious:   与每个anchor iou最大的gt框与其的iou值
                        gt_argmax_ious:  与每个gt_box 最大iou的anchor序号
        """
        #----------------------------------------------#
        #   anchor和bbox的iou
        ...
        #---------------------------------------------------------#
        #   获得每一个anchor框最对应的真实框的序号  [num_anchors, ]
        #---------------------------------------------------------#
        argmax_ious = ious.argmax(axis=1)
        #---------------------------------------------------------#
        #   找出每一个anchor框与最对应的真实框的iou  [num_anchors, ]
        #---------------------------------------------------------#
        max_ious = np.max(ious, axis=1)
        
        #---------------------------------------------------------#
        #   获得每一个gt框最对应的anchor框的序号  [num_gt, ]
        #---------------------------------------------------------#
        gt_argmax_ious = ious.argmax(axis=0)

        #---------------------------------------------------------#
        #   保证每一个gt框都与与其iou最大的anchor匹配
        #---------------------------------------------------------#
    ...
    def _create_label(self, anchor, bbox):
        ...
        :param anchor:
        :param bbox:
        :return:                argmax_ious   (n_anchors,)  与每个anchor iou值最大的gt框的编号
                                label         (n_ahchors,)  每一个anchor属于正负样本的编号  正样本(1),负样本(0),丢弃样本(-1)
        """
        # ------------------------------------------ #
        #   1是正样本，0是负样本，-1忽略
        #   初始化的时候全部设置为-1
        ...
        # ------------------------------------------------------------------------ #
        #   argmax_ious:与每个anchor框对应的最大的gt框的序号         [num_anchors, ]
        #   max_ious:与每个真实框对应的最大的gt框的iou值             [num_anchors, ]
        #   gt_argmax_ious:与每一个gt框对应的最大的anchor框的序号    [num_gt, ]
        # ------------------------------------------------------------------------ #
